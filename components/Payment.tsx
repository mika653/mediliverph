...
export const Payment: React.FC<Props> = ({ amount, onSuccess, onCancel }) => {
  ...
  const methods = [
    ...
    { id: 'card', name: 'Credit/Debit Card', icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>
    )},
  ...
  return (
    <div className="p-6 bg-white rounded-3xl shadow-xl max-w-md mx-auto border border-blue-100/50">
      <h2 className="font-display text-3xl text-stone-800 mb-2 text-center">Checkout</h2>
      <p className="text-stone-400 text-center mb-8 text-lg">Monthly Subscription: <span className="font-bold text-blue-600">{amount}</span></p>
            ...
            {methods.map(m => (
              <button
                ...
                className={`flex items-center gap-4 p-5 rounded-2xl border-2 transition-all text-lg ${
                  method === m.id ? 'border-blue-500 bg-blue-50 shadow-sm' : 'border-stone-100 hover:border-blue-200'
                }`}
              >
                ...
                {method === m.id && (
                  <span className="ml-auto">
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                  </span>
                )}
              </button>
            ))}
          ...
          <div className="mt-8 flex flex-col gap-3">
            <button
              ...
              className={`w-full py-5 rounded-2xl text-2xl font-bold transition-all shadow-lg ${
                method ? 'bg-blue-600 text-white hover:bg-blue-700 hover:shadow-xl' : 'bg-stone-200 text-stone-400 cursor-not-allowed'
              }`}
            >
              Pay Now
            </button>
